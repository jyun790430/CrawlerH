...
class XvideosPipeline(object):
    ...
    def process_item(self, item, spider):
        _sql = """
            INSERT INTO crawl_video (type, name, tags, file_name, origin_url, unique_token, duration) 
            VALUES (\"%s\", \"%s\",\"%s\", \"%s\",\"%s\",\"%s\",\"%s\")
        """ % (
            item['type'], item['name'], item['tags'], item['file_name'], item['origin_url'], item['unique_token'], item['duration']
        )
        try:
